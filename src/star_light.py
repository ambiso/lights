...
def make_sparkle_cache(n, brightness: float):
    ...
    return [_sparkle(
            np.array(colorsys.hsv_to_rgb(i / (n-1), 1, 1))
    ) for i in range(n)]
...
def sparkle(strip, get_current_brightness = lambda: 1.):
    ...
    time = 0
    fill(strip, sparkle_cache[time][-1])
    ...
    while True:
        ...
        for a in sparkles:
            strip.setPixelColor(a[0], sparkle_cache[time][a[1]])
            a[1] += 1
        strip.show()
        sparkles = [[pos, time] for pos, time in sparkles if time < len(sparkle_cache[time])]
        time += 1
        time %= n